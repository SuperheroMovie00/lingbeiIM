package com.haitaoit.pinpai.add.activity.fragment;
...
import com.haitaoit.pinpai.add.controller.ContactsController;
...
import com.haitaoit.pinpai.add.entity.FriendInvitation;
import com.haitaoit.pinpai.utils.SharePreferenceManager;
import com.haitaoit.pinpai.utils.ThreadUtil;
import com.haitaoit.pinpai.utils.pinyin.HanziToPinyin;
import com.haitaoit.pinpai.view.ContactsView;
...
public class ContactsFragment extends BaseFragment {
    ...
    public void onResume() {
        ...
        //为搜索群组做准备
        ExitApplication.mGroupInfoList.clear();
                            ...
                            for (final Long groupID : groupIDList) {
                                JMessageClient.getGroupInfo(groupID, new GetGroupInfoCallback() {
                                    ...
                                    public void gotResult(int responseCode, String responseMessage, GroupInfo groupInfo) {
                                        if (responseCode == 0) {
                                            ExitApplication.mGroupInfoList.add(groupInfo);
                                        }
                                    }
                                });
                            }
        ...
        //为搜索好友做准备
        if (ExitApplication.mFriendInfoList != null)
            ExitApplication.mFriendInfoList.clear();
        ContactManager.getFriendList(new GetUserInfoListCallback() {
            @Override
            public void gotResult(int i, String s, List<UserInfo> list) {
                if (i == 0) {
                    ExitApplication.mFriendInfoList = list;
                }
            }
        });
    }
    ...
    public void onEvent(ContactNotifyEvent event) {
        final UserEntry user = ExitApplication.getUserEntry();
        ...
        if (event.getType() == ContactNotifyEvent.Type.invite_accepted) {
            ...
            ExitApplication.forAddFriend.remove(username);
            FriendRecommendEntry entry = FriendRecommendEntry.getEntry(user, username, appKey);
            ...
            if (ExitApplication.forAddFriend.size() > 0) {
                for (String forAdd : ExitApplication.forAddFriend) {
                    if (forAdd.equals(username)) {
                        return;
                    } else {
                        ExitApplication.forAddFriend.add(username);
                    }
                }
            } else {
                ExitApplication.forAddFriend.add(username);
