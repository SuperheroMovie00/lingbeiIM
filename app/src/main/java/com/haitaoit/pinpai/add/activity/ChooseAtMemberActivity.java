...
import com.haitaoit.pinpai.add.adapter.AtMemberAdapter;
import com.haitaoit.pinpai.ExitApplication;
import com.haitaoit.pinpai.add.model.ParentLinkedHolder;
import com.haitaoit.pinpai.utils.keyboard.widget.EmoticonsEditText;
import com.haitaoit.pinpai.utils.pinyin.UserComparator;
import com.haitaoit.pinpai.utils.sidebar.SideBar;
import com.haitaoit.pinpai.view.listview.StickyListHeadersListView;
...
public class ChooseAtMemberActivity extends BaseActivity {
    ...
    public static void show(ChatActivity textWatcher, EmoticonsEditText editText, String targetId) {
        ...
        intent.putExtra(ExitApplication.GROUP_ID, Long.parseLong(targetId));
        textWatcher.startActivityForResult(intent, ExitApplication
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        long groupId = getIntent().getLongExtra(ExitApplication.GROUP_ID, 0);
        if (0 != groupId) {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                ...
                synchronized (ChooseAtMemberActivity.class) {
                    if (textParentLinkedHolder != null) {
                        ...
                        if (editText != null)
                            intent.putExtra(ExitApplication.NAME, atName);
                    }
                }
                ...
                setResult(ExitApplication.RESULT_CODE_AT_MEMBER, intent);
                finish();
            }
        });
        ...
        mLl_groupAll.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //@all
                Intent intent = new Intent();
                intent.putExtra(ExitApplication.ATALL, true);
                setResult(ExitApplication.RESULT_CODE_AT_ALL, intent);
                finish();
            }
        });
        ...
        mSearch_title.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ExitApplication.mSearchAtMember = mList;
                startActivityForResult(intent, ExitApplication.SEARCH_AT_MEMBER_CODE);
            }
        });
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 33 && data != null) {
            Intent intent = new Intent();
            intent.putExtra(ExitApplication.NAME, data.getStringExtra(ExitApplication.SEARCH_AT_MEMBER_NAME));
            intent.putExtra(ExitApplication.TARGET_ID, data.getStringExtra(ExitApplication.SEARCH_AT_MEMBER_USERNAME));
            intent.putExtra(ExitApplication.TARGET_APP_KEY, data.getStringExtra(ExitApplication.SEARCH_AT_APPKEY));
            setResult(ExitApplication.RESULT_CODE_AT_MEMBER, intent);
            finish();
        }
    }
