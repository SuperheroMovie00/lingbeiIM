...
import com.haitaoit.pinpai.add.controller.ChatDetailController;
...
public class ChatDetailActivity extends BaseActivity {
    ...
    public void onBackPressed() {
        ...
        intent.putExtra(ExitApplication.CONV_TITLE, mChatDetailController.getName());
        intent.putExtra(ExitApplication.MEMBERS_COUNT, mChatDetailController.getCurrentCount());
        intent.putExtra("deleteMsg", mChatDetailController.getDeleteFlag());
        setResult(ExitApplication.RESULT_CODE_CHAT_DETAIL, intent);
        finish();
        super.onBackPressed();
    }
            ...
            switch (requestCode) {
                ...
                case MY_NAME_REQUEST_CODE:
                    if (data.getBooleanExtra("returnChatActivity", false)) {
                        ...
                        data.putExtra(ExitApplication.NAME, mChatDetailController.getName());
                        setResult(ExitApplication.RESULT_CODE_CHAT_DETAIL, data);
                        finish();
                    }
                    break;
                case ExitApplication.REQUEST_CODE_ALL_MEMBER:
    ...
    public void startChatActivity(long groupID, String groupName, int member) {
        ...
        intent.putExtra(ExitApplication.MEMBERS_COUNT, member + 1);
        intent.putExtra(ExitApplication.GROUP_ID, groupID);
        intent.putExtra(ExitApplication.CONV_TITLE, groupName);
    ...
    public void onEvent(MessageEvent event) {
        ...
        if (msg.getContentType() == ContentType.eventNotification) {
            ...
            handleMsg.what = ExitApplication.ON_GROUP_EVENT;
            Bundle bundle = new Bundle();
            bundle.putLong(ExitApplication.GROUP_ID, ((GroupInfo) msg.getTargetInfo()).getGroupID());
    ...
    private static class UIHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (activity != null) {
                switch (msg.what) {
                    case ExitApplication.ON_GROUP_EVENT:
                        activity.mChatDetailController.refresh(msg.getData()
                                .getLong(ExitApplication.GROUP_ID, 0));
                        break;
                }
            }
        }
