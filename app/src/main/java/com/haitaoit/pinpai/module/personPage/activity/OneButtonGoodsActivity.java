...
import com.haitaoit.pinpai.module.personPage.adapter.OneButtonGoodsAdapter;
...
public class OneButtonGoodsActivity extends BaseActivity {
    ...
    MyTextView tvShare;
    private OneButtonGoodsAdapter mmOneButtonGoodsAdapter;
    //记录选择的Item
    ...
    private void initCollList(final boolean isRefresh) {
        ...
        ApiRequest.GetUersGoodsList(map, new MyCallBack<GetUserGooddListObj>(mContext) {
            @Override
            public void onSuccessful(GetUserGooddListObj response) {
                if (response.getErrCode() == 0) {
                    ...
                    mmOneButtonGoodsAdapter.notifyDataSetChanged();
                    currentPage++;
    ...
    private void initAdapter() {
        ...
        rvCollecView.setAdapter(mmOneButtonGoodsAdapter = new OneButtonGoodsAdapter(mContext, mDataList));
        rvCollecView.setNestedScrollingEnabled(false);
        mmOneButtonGoodsAdapter.setOnItemClickListener(new OneButtonGoodsAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
                Bundle bundle = new Bundle();
                bundle.putString("supplyId", mDataList.get(position).getGoods_id());
                RxActivityUtils.skipActivityAndFinish(mContext, SupplyGoodsDetailActivity.class, bundle);
            }

        });
        mmOneButtonGoodsAdapter.setCallback(new OneButtonGoodsAdapter.CartEventCallback() {
            @Override
        ...
        switch (view.getId()) {
            case R.id.tv_select_all:
                if (tvSelectAll.isChecked()) {
                    mmOneButtonGoodsAdapter.setSelectAll(true);
                    mmOneButtonGoodsAdapter.notifyDataSetChanged();
                } else {
                    mmOneButtonGoodsAdapter.setSelectAll(false);
                    mmOneButtonGoodsAdapter.notifyDataSetChanged();
                }
                break;
            case R.id.tv_share:
    ...
    private void initShare() {
        List<String> selectPos = mmOneButtonGoodsAdapter.getSelectPos();
        if (selectPos.size() == 0) {
...
//        photoDialog.setCanceledOnTouchOutside(true);
    ...
    private void initVisibleShare() {
        ...
        ApiRequest.PostGoodsShare(map, item, new MyCallBack<ResponseObj>(mContext) {
            @Override
